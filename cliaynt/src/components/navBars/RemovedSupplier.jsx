...
function RemovedSupplier() {
    ...
    const fetchData = async () => {
        try {
            const result = await api.get(`/admin/removed-supplier?page=${page}&limit=10`);
            if (result.data.status) {
                setRemovedSuppliers(result.data.result);
            ...
            Swal.fire({
                ...
                .then(async (result) => {
                    if (result.isConfirmed) {
                        const response = await api.put(`/admin/recycle-supplier/${id}`)
                        if (response.data.status) {
                            fetchData()
                            Swal.fire({
                                title: "Recycle!",
                                text: "supplier file has been recycle.",
                                icon: "success"
                            });
                        }
                    }
    ...
    return (
        <div className="p-4 mt-16 bg-white rounded-lg shadow ">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Supplier</h2>

            {/* Desktop View */}
            <div className=" overflow-x-auto" id="removed-supplier-table">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {removedSuppliers.length > 0 ? removedSuppliers.map((c, index) => (
                                    <tr key={c.id || index} className="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <td className="py-4 px-4 text-gray-900">{c.companyName}</td>
...
export default RemovedSupplier
